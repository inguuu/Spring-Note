...
자바소스파일을 컴파일한 후 생성된 바이트코드를 기계어로 변환하여 자바명령문을 실행시킴
...
하나의 메소드만 선언된 함수형 인터페이스 구현할때만 가능
...
java.lang.Thread클래스 상속없이 구현 가능


## throws

해당 메소드를 호출한 곳으로 발생한 오류를 처리하도록 선언하는 키워드
